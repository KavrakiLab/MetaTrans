...
def build_optim(model, opt, checkpoint):
    ...
    # in order to resume training from the configuration of the last checkpoint:
    if opt.resume_training:
...
class Optimizer(object):
    ...
    def step(self):
        ...
        # Decay based on start_decay_steps every decay_steps
            ...
            if (self.start_decay_steps is not None):
                if self._step < self.start_decay_steps:
                    self.learning_rate = self.original_lr * (self._step/self.start_decay_steps)
                else:
                    self.learning_rate = self.original_lr * (self.train_steps-self._step)/(self.train_steps-self.start_decay_steps)
