...
def main(opt, device_id):
    ...
    if opt.resume_training:
        logger.info('Loading checkpoint from %s' % opt.resume_training)
        checkpoint = torch.load(opt.resume_training,
    ...
    if opt.train_from:
        ...
        for i in range(0, len(checkpoint['optim'].optimizer.param_groups)):
            ...
            checkpoint['optim'].optimizer.param_groups[i]['warmup_steps'] = opt.warmup_steps
    else:
        checkpoint = None
        model_opt = opt
    ...
    # Build optimizer.
    # if opt.resume_training:
    #     optim = build_optim(model, opt, checkpoint)  # in order to resume training from the configuration of the last checkpoint
    # else:
